"""V11: Error propagation features + region pooling + v10 enhanced features.

Two new ideas on top of v10:
1. Error propagation: use d1 model's predicted interval width as a feature for d7/d14.
   Wide d1 intervals → more uncertain → wider d7/d14 intervals.
2. Region pooling: train one model for both regions with region as a categorical feature.
   2x more data, shared physical relationships.

Requires: run_v10.py must have been run first (enhanced features + direction + speed models).

Usage: python src/pipeline/run_v11.py
"""
...
SAVE_DIR_SPEED = LOGS_DIR / "speed_models_v11"
SAVE_DIR_DIR = LOGS_DIR / "direction_models_v11"
CQR_DIR = LOGS_DIR / "cqr_v11"
...
def add_enhanced_features(df, region=None, window_id=None):
    df = df.copy()
    ...
    if geo_df is not None and not geo_df.empty and "latitude" in geo_df.columns:
        ...
        if geo_cols and "latitude" in df.columns:
...
def add_d1_width_feature(df, level, horizon):
    if horizon == 1:
        return df
    if level in ("10m", "100m"):
        d1_q05 = df.get("speed_d1_h0")
        d1_q95 = df.get("speed_d1_h0")
        if d1_q05 is not None:
            df["d1_forecast_range"] = np.abs(df.get("speed_d1_h12", df.get("speed_d1_h0", pd.Series(0))) - df.get("speed_d1_h0", pd.Series(0)))
    else:
        u_d1 = df.get(f"fcst_u_{level}_d1_h0")
        v_d1 = df.get(f"fcst_v_{level}_d1_h0")
        u_d7 = df.get(f"fcst_u_{level}_d7_h0")
        v_d7 = df.get(f"fcst_v_{level}_d7_h0")
        if u_d1 is not None and u_d7 is not None:
            spd_d1 = speed_from_uv(u_d1.values, v_d1.values)
            spd_d7 = speed_from_uv(u_d7.values, v_d7.values)
            df["d1_d7_forecast_diff"] = np.abs(spd_d1 - spd_d7)
    return df
...
def get_feature_cols_enhanced(columns):
    ...
    extra = [
        ...
        "msl_tendency", "dist_to_coast_km", "lsm",
        "d1_forecast_range", "d1_d7_forecast_diff",
    ]
...
def load_pooled_features():
    ...
    for region in REGIONS:
        ...
        feat = add_enhanced_features(feat, region=region)
        feat["region_ns"] = int(region == "north_sea")
        feat["region_ecs"] = int(region == "east_china_sea")
        frames.append(feat)
    return pd.concat(frames, ignore_index=True)


def load_pooled_reanalysis(level):
    frames = []
    for region in REGIONS:
        rean = load_reanalysis_level(region, level)
        rean["region_ns"] = int(region == "north_sea")
        rean["region_ecs"] = int(region == "east_china_sea")
        frames.append(rean)
    return pd.concat(frames, ignore_index=True)


def train_direction_v11():
    print("=" * 60)
    print("V11: Training per-horizon direction models (enhanced + error propagation)")
    print("=" * 60)
    ...
    for region in REGIONS:
        ...
        for level in ALL_LEVELS:
            ...
            for horizon in HORIZONS:
                ...
                print(f"  {region}/{level}/d{horizon}: tune cWS={cws:.1f}")
    ...
    print(f"Direction: {len(results)} models trained")


def predict_direction_v11():
    all_dir = {}
    for region in REGIONS:
        for level in ALL_LEVELS:
            for horizon in HORIZONS:
                ...
                for wid in range(1, 9):
                    ...
                    for hour in HOURS:
                        df_pred = inf_enhanced[[c for c in feat_cols if c in inf_enhanced.columns]].copy()
                        for c in feat_cols:
...
def train_speed_pooled_v11():
    print("\n" + "=" * 60)
    print("V11: Training REGION-POOLED residual speed models (enhanced features)")
    print("=" * 60)
    SAVE_DIR_SPEED.mkdir(parents=True, exist_ok=True)

    pooled = load_pooled_features()
    feat_cols = get_feature_cols_enhanced(pooled.columns)
    feat_cols += ["region_ns", "region_ecs"]
    feat_cols = sorted(set(feat_cols))

    if len(pooled) > 400_000:
        pooled = pooled.sample(n=400_000, random_state=42).reset_index(drop=True)

    print(f"  Pooled features: {len(feat_cols)} columns, {len(pooled):,} rows")

    for level in ALL_LEVELS:
        rean = load_pooled_reanalysis(level)
        ...
        for horizon in HORIZONS:
            ...
            for hour in HOURS:
                ...
                merged = fm.merge(
                    ...
                    on=["target_time", "latitude", "longitude", "region_ns", "region_ecs"],
                    how="inner",
                ...
                if horizon > 1:
                    merged = add_d1_width_feature(merged, level, horizon)
            ...
            tr, vl, tu, ho = split_by_time(df)
            print(f"  {level}/d{horizon}: train={len(tr):,} val={len(vl):,} (pooled)")

            if len(tr) < 200:
                continue

            final_feat_cols = sorted(set(feat_cols) | {c for c in df.columns if c.startswith("d1_")})

            X_tr = add_time_encodings(tr[final_feat_cols + ["time"]].copy())
            ...
            X_vl = add_time_encodings(vl[final_feat_cols + ["time"]].copy()) if len(vl) > 0 else None
            if X_vl is not None:
            ...
            all_model_cols = sorted(set(X_tr.columns) | (set(X_vl.columns) if X_vl is not None else set()))
            X_tr = align_columns(X_tr, all_model_cols)
            X_vl = align_columns(X_vl, all_model_cols) if X_vl is not None else None
            ...
            sp = SAVE_DIR_SPEED / "pooled" / level / f"d{horizon}"
            sp.mkdir(parents=True, exist_ok=True)
            with open(sp / "model.pkl", "wb") as f:
                pickle.dump({
                    "models": models, "feature_cols": all_model_cols,
                    "level": level, "horizon": horizon, "pooled": True,
                }, f)
...
def compute_cqr_v11():
    ...
    for level in ALL_LEVELS:
        for horizon in HORIZONS:
            mp = SAVE_DIR_SPEED / "pooled" / level / f"d{horizon}" / "model.pkl"
            if not mp.exists():
                continue
            for region in REGIONS:
    ...
    print(f"  Using zero offsets (CQR deferred): {len(offsets)} entries")


def generate_v11():
    print("\n" + "=" * 60)
    print("Generating V11 submission")
    print("=" * 60)
        ...
        for level in ALL_LEVELS:
            for horizon in HORIZONS:
                ...
                for wid in range(1, 9):
                    ...
                    inf["region_ns"] = int(region == "north_sea")
                    inf["region_ecs"] = int(region == "east_china_sea")
                    if horizon > 1:
                        inf = add_d1_width_feature(inf, level, horizon)

                    for hour in HOURS:
                        avail = [c for c in feat_cols if c in inf.columns]
                        X = inf[avail].copy()
                        for c in feat_cols:
    ...
    save_submission(submission, "v11")


if __name__ == "__main__":
    t0 = time.time()
    train_direction_v11()
    train_speed_pooled_v11()
    compute_cqr_v11()
    generate_v11()
    print(f"\nV11 done in {(time.time()-t0)/60:.1f} min")
