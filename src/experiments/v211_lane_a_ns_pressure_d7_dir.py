"""v211: Lane A scaling — von Mises kappa-MLE direction widths on NS pressure d7.

NS variant of v209 (which targeted ECS). Same mechanism, same regime gate,
same auto-ship gates. Only difference: REGION="north_sea".

Pilot scope:
  type=grid, region=north_sea, level in {1000, 925, 850, 700, 500},
  horizon=7, hour in {0,6,12,18}. 20 cells total.

Base: v207. Will OVERWRITE v202 width-shrink on this cell. If Lane A is
structurally better than the speed*hw heuristic, v211 should beat v207's 263.20.

Build only if v209 (ECS variant) validates Lane A on hidden 2022 test.
...
OUT_DIR = output_dir(VERSION, "lane_a_ns_pressure_d7_dir")
