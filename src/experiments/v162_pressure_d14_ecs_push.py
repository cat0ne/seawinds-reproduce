"""v162: v156-based ECS pressure d14 speed blend push.

v56/v57 proved the ECS pressure d14 shear/ws3 speed family can transfer hidden
when it is conservative and level-gated. Current v156 already contains the v57
shape: global 0.20 blend toward v55 on ECS pressure d14, with levels 850/925
upgraded to 0.35. This probe keeps that same hidden-proven level gate and raises
only levels 850/925 to 0.50.

Scope:
- base: v156
- anchor/donor family: v53 -> v55 pressure-shear/ws3 predictions
- changed rows: grid / east_china_sea / pressure levels 850, 925 / horizon d14
...
OUT_DIR = LOGS_DIR / "v162_pressure_d14_ecs_push"

REGION = "east_china_sea"
TARGET_HORIZON = 14
UPGRADE_LEVELS = ["850", "925"]
STRONG_BLEND_LAMBDA = 0.50
Q_COLS = ["q05", "q50", "q95"]
...
def repair_quantiles(frame: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    out = frame.copy()
    q = out.loc[mask, Q_COLS].to_numpy(float)
    q = np.sort(q, axis=1)
...
def build_candidate() -> tuple[pd.DataFrame, dict]:
    ...
    for level, group_idx in base.loc[target].groupby(base.loc[target, "level"].astype(str)).groups.items():
        ...
        row = {
            ...
            "width_delta": float((cand_w - base_w).mean()),
        }
        for col in Q_COLS:
            row[f"mean_delta_{col}"] = float((out.loc[idx, col].astype(float) - base.loc[idx, col].astype(float)).mean())
