...
Key differences vs. the original heavy notebook:
  ...
  * REGIONS = ["east_china_sea"] only (cuts cost in half).
  * Model checkpointing under logs/heavy_ecs/{region}/...:
  ...
  * Two inference modes: --infer-hidden (8 windows → outputs/heavy_ecs_hidden.csv)
    ...
    replay_base/v59_surface_d7_ecs.parquet, keyed by target_time).
...
OUTPUT_HIDDEN_CSV_REPLAY = PROJECT_ROOT / "outputs" / "heavy_ecs_hidden_replay.csv"
OUTPUT_HIDDEN_CSV_BYTEEQUAL = PROJECT_ROOT / "outputs" / "heavy_ecs_hidden_byteequal.csv"
REPLAY_PARQUET = PROJECT_ROOT / "replay_base" / "v59_surface_d7_ecs.parquet"
...
def compute_or_load_selection(
    ...
    *,
    ...
    on the truncated window and caches under logs/heavy_ecs/."""
