...
[`../scripts/`](../scripts/) are the build trace: the DAG, every artifact's `sha256`, and
...
> **Note on the originals.** The scripts under `scripts/` are the project's
...
```bash
python scripts/reproduce_v222_plus_v227_plus_v232.py --mode full   # rebuild v53→v232 from the cached lineage
python scripts/reproduce_v222_plus_v227_plus_v232.py               # verify-only: re-hashes the stored base
```

The production base is **sha-pinned** (`13c9fa05…`). The **full lineage code is included** in
this package: the `v51 → … → v232` producer modules live under [`../src/`](../src/) (the engine)
and the driver under [`../scripts/`](../scripts/) (scrubbed of competitor leaderboard data; the
exact set is the import closure of the chain). Each overlay is a single-cell, center-frozen
width change, recorded with its checksum in [`../CHECKSUMS.md`](../CHECKSUMS.md). The lineage
root `v51` is itself rebuildable from raw — see [`END_TO_END.md`](END_TO_END.md) and
[`../scripts/rebuild_v51.py`](../scripts/rebuild_v51.py). A full cold re-run needs the 21 GB
dataset and is **library-version sensitive** (see §7); this is why the production base and the
final artifact are sha-pinned rather than assumed byte-reproducible from a fresh environment.
...
- Notebook: `scripts/heavy/2d_starting_kit_heavy.ipynb`
- Plain-Python extraction: `scripts/heavy/_heavy_extracted.py`
